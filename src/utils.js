 ...
 */
const getFilename = path => path.replace(/.*((\/|\\)([-_a-zA-Z0-9.]+))$/, '$3');
 ...
 * @description return floder paths contained in `inputPaths`
 * @param       {String[]}           inputPaths    list of file paths
 * @param       {String}             rootPath      path to append to all paths
 * @return      {String[]}                         folder paths list
 */
const getFolderPaths = (inputPaths, rootPath) => {
  const folderPaths = [];
  for (const inputPath of inputPaths) {
    const paths = getPathsOrPath(rootPath ? path.join(rootPath, inputPath) : inputPath);

    if (Array.isArray(paths)) folderPaths.push(inputPath);
  }
